...
public class NotesList extends ListActivity {
    ...
    public static int[] lockViewAnswer;
    ...
    private static final String[] PROJECTION = new String[]{NotePad.Notes._ID, // 0
            ...
            NotePad.Notes.COLUMN_NAME_CREATE_DATE, NotePad.Notes.COLUMN_NAME_MODIFICATION_DATE,//4

    };
    private static final String[] SERACHITEM = new String[]{NotePad.Notes.COLUMN_NAME_TITLE};
    ...
    protected void onCreate(Bundle savedInstanceState) {
         ...
         */
        Cursor cursor = managedQuery(getIntent().getData(),            // Use the default content
                // URI for the provider.
                PROJECTION,                       // Return the note ID and title for each note.
                null,                             // No where clause, return all records.
                null,                             // No where clause, therefore no where column
                // values.
        ...
        String[] dataColumns = {NotePad.Notes.COLUMN_NAME_TITLE, NotePad.Notes
        ...
        // Creates the backing adapter f or the ListView.
        SimpleCursorAdapter adapter = new SimpleCursorAdapter(this,
                // The Context for the ListView
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        menu.addIntentOptions(Menu.CATEGORY_ALTERNATIVE, 0, 0, new ComponentName(this, NotesList
                .class), null, intent, 0, null);
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (haveItems) {
             ...
             */
            menu.addIntentOptions(Menu.CATEGORY_ALTERNATIVE,  // Add the Intents as options in
                    // the alternatives group.
            ...
            case R.id.menu_search_input:
                ...
                new AlertDialog.Builder(NotesList.this).setTitle("输入查询").setView(et)
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (doSearch(input_word))//cursor
                        {
                            startActivity(new Intent(Intent.ACTION_EDIT, mUri));
                        } else {
                            Toast.makeText(NotesList.this, "不存在的", Toast.LENGTH_SHORT).show();
                        }
                    }
                }).show();
    ...
    /*public static int[] int2Array(int a) {
        String b = a + "";
        int[] result = new int[b.length()];
        for (int i = 0; i < b.length(); i++) {
            result[i] = b.charAt(i) - '0';
            if (result[i] == 0) {
                Toast.makeText(MyApplication.getContext(), "保存密码出现数字0,恢复默认密码:1", Toast
                        .LENGTH_LONG).show();
                return new int[]{1};//如果数字为零，恢复默认密码. （输入限制不可能为负数）
            }
        }
        for (int i = 0; i < b.length() - 1; i++)
            for (int j = i + 1; j < b.length(); j++) {
                if (result[i] == result[j]) {
                    Toast.makeText(MyApplication.getContext(), "保存密码数字重复,恢复默认密码:1", Toast
                            .LENGTH_LONG).show();
                    return new int[]{1};
                }
            }//出现重复数字，也恢复默认密码
    ...
    public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menuInfo) {
        ...
        Intent intent = new Intent(null, Uri.withAppendedPath(getIntent().getData(), Integer
                .toString((int) info.id)));
        intent.addCategory(Intent.CATEGORY_ALTERNATIVE);
        menu.addIntentOptions(Menu.CATEGORY_ALTERNATIVE, 0, 0, new ComponentName(this, NotesList
                .class), null, intent, 0, null);
    }
